...
export function ExcelWorkbookView({
  ...
  const autoCalculatedSequence = React.useMemo(() => {
    const matchingDocs = docList.filter((d) => {
      return (
  ...
  }, [docList, selectedGroupCode, customType, currentGroupInfo]);
  ...
  const isDuplicateDocNo = React.useMemo(() => {
    return docList.some((d) => d.documentNo.toUpperCase() === autoGeneratedDocNo.toUpperCase());
  }, [docList, autoGeneratedDocNo]);
  ...
  const handleSaveNewDoc = async (e: React.FormEvent) => {
    ...
    const newCreated = await createDocMutation.mutateAsync({
      projectCode: "CM24045",
      originatorCode: "EPS",
  ...
  }, [docList, disciplineFilter, conzolFilter, searchQuery]);

  // Metrics
  const pendingCount = docList.filter((d) => !d.erpSynced).length;
  const uploadedCount = docList.filter((d) => d.erpSynced).length;
      ...
      <div className="flex items-center justify-between border-t border-slate-300 bg-[#e7e6e6] px-2 py-1 text-xs select-none overflow-x-auto">
        ...
        <div className="flex items-center space-x-1 shrink-0">
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3 py-1 rounded-t font-bold text-xs transition-all border-b-2 cursor-pointer ${
              activeTab === "Disc"
          ...
          <button
            ...
            className={`px-3 py-1 rounded-t font-bold text-xs transition-all border-b-2 cursor-pointer ${
              activeTab === "Coding"
                ? "bg-white text-[#107c41] border-[#107c41] shadow-xs"
                : "text-slate-700 hover:bg-slate-200 border-transparent"
            }`}
            ...
            <form onSubmit={handleSubmitRevision} className="space-y-3 text-xs">
              ...
              <div className="flex items-center justify-end space-x-2 pt-2 border-t">
                <button
                  type="button"
                  onClick={() => setRevisionDoc(null)}
                  className="px-3 py-1.5 rounded border border-slate-300 text-slate-600 hover:bg-slate-100 text-xs font-medium cursor-pointer"
                >
                  ยกเลิก
                </button>
                <button
                  ...
                  className="px-4 py-1.5 rounded bg-[#107c41] hover:bg-[#0e6b37] text-white font-bold text-xs shadow-xs cursor-pointer"
                >
